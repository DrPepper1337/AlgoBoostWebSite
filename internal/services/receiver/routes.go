...
func SetupRoutes(db *database.Database) http.Handler {
	...
	r.Use(cors.Handler(cors.Options{
		AllowedOrigins:   []string{"http://localhost:5173", "http://localhost:8080", "https://algoboost.foo", "http://localhost:8088"},
	...
	r.With(middleware.AdminMiddleware).Post("/api/admin/upload-tests/{taskId}", UploadTestsHandler(db))
